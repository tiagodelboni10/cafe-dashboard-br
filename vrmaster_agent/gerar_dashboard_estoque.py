...
@media print{{body{{background:#fff;color:#000;padding:0}}.hdr,.nav,.fl,.scroll-top,.modal-overlay{{display:none}}.pane{{display:block!important;border:none;padding:10px 0;background:#fff}}table{{font-size:9pt}}th{{background:#eee;color:#000}}td{{border-color:#ddd}}.b{{border:1px solid #999;background:#f0f0f0;color:#000}}}}
</style>
</head>
<body>
...
var _setKeys=[];
function rSet(){{var c=document.getElementById('cds-set'),entries=Object.entries(SET);_setKeys=entries.map(e=>e[0]);c.innerHTML='';entries.forEach((e,i)=>{{var n=e[0],s=e[1],t=s.total||1;var d=document.createElement('div');d.className='cd';d.setAttribute('data-idx',i);d.innerHTML='<h3>'+n+' <span style="font-size:.7em;color:#666">&#9660;</span></h3><div class="sts"><span><b style="color:var(--danger)">'+s.ruptura+'</b> rupt</span><span><b style="color:var(--warn)">'+s.alerta+'</b> alerta</span><span><b style="color:var(--ok)">'+s.ok+'</b> ok</span><span><b style="color:var(--excess)">'+s.excesso+'</b> exc</span></div><div style="font-size:.8em;color:var(--muted)">'+s.total+' itens | '+R(s.valor_estoque)+'</div>'+(s.valor_excesso>0?'<div style="font-size:.8em;color:var(--excess)">'+R(s.valor_excesso)+' excesso</div>':'')+'<div class="br"><div style="width:'+(s.ruptura/t*100)+'%;background:var(--danger)"></div><div style="width:'+(s.alerta/t*100)+'%;background:var(--warn)"></div><div style="width:'+(s.ok/t*100)+'%;background:var(--ok)"></div><div style="width:'+(s.excesso/t*100)+'%;background:var(--excess)"></div></div><div class="detail-panel" style="display:none"></div>';d.addEventListener('click',function(){{abrirModal('setor',_setKeys[this.getAttribute('data-idx')])}});c.appendChild(d)}})}}
var _frnKeys=[];
...
function rFL(e){{var c=document.getElementById('cds-forn');_frnKeys=e.map(x=>x[0]);c.innerHTML='';e.forEach((entry,i)=>{{var n=entry[0],f=entry[1],t=f.total||1;var d=document.createElement('div');d.className='cd';d.setAttribute('data-idx',i);d.innerHTML='<h3>'+n+' <span style="font-size:.7em;color:#666">&#9660;</span></h3><div class="sts"><span><b style="color:var(--danger)">'+f.ruptura+'</b> rupt</span><span><b style="color:var(--warn)">'+f.alerta+'</b> alerta</span><span><b style="color:var(--ok)">'+f.ok+'</b> ok</span><span><b style="color:var(--excess)">'+f.excesso+'</b> exc</span></div><div style="font-size:.8em;color:var(--muted)">'+f.total+' itens | ~'+f.lead_time+'d entrega | '+R(f.valor_estoque)+'</div><div class="br"><div style="width:'+(f.ruptura/t*100)+'%;background:var(--danger)"></div><div style="width:'+(f.alerta/t*100)+'%;background:var(--warn)"></div><div style="width:'+(f.ok/t*100)+'%;background:var(--ok)"></div><div style="width:'+(f.excesso/t*100)+'%;background:var(--excess)"></div></div><div class="detail-panel" style="display:none"></div>';d.addEventListener('click',function(){{abrirModal('forn',_frnKeys[this.getAttribute('data-idx')])}});c.appendChild(d)}})}}
function abrirModal(tp,nm){{
 ...
 const nR=prods.filter(x=>x.status==='ruptura'||x.status==='negativo').length;
 const nA=prods.filter(x=>x.status==='alerta').length;
 const nO=prods.filter(x=>x.status==='ok').length;
 document.getElementById('modal-title').textContent=nm+' ('+prods.length+' itens)';
 document.getElementById('modal-body').innerHTML=
  '<div style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:14px;font-size:.88em"><span><b style="color:var(--danger)">'+nR+'</b> ruptura</span><span><b style="color:var(--warn)">'+nA+'</b> alerta</span><span><b style="color:var(--ok)">'+nO+'</b> ok</span></div>'+
  '<div class="tw"><table class="resp"><thead><tr><th>Status</th><th>Curva</th><th>Cod.</th><th>Produto</th>'+(tp==='setor'?'<th>Categoria</th><th>Fornecedor</th>':'<th>Setor</th><th>Categoria</th>')+'<th>Estoque</th><th>VMD</th><th>DDV</th><th>Fat.30d</th><th>Ult.Entrada</th></tr></thead><tbody>'+
  prods.map(x=>'<tr class="'+({{ruptura:'R',alerta:'A',excesso:'E',negativo:'N'}}[x.status]||'')+'"><td data-label="Status">'+sb(x.status)+'</td><td data-label="Curva">'+cb(x.curva)+'</td><td data-label="Codigo">'+x.cod+'</td><td data-label="Produto" class="td-main">'+x.desc+'</td>'+(tp==='setor'?'<td data-label="Categoria">'+x.cat+'</td><td data-label="Fornecedor">'+x.forn+'</td>':'<td data-label="Setor">'+x.setor+'</td><td data-label="Categoria">'+x.cat+'</td>')+'<td data-label="Estoque">'+x.est.toFixed(1)+'</td><td data-label="VMD">'+x.vmd.toFixed(1)+'</td><td data-label="DDV">'+dv(x.ddv,x.ddv_ideal,x.lt)+'</td><td data-label="Fat.30d">'+R(x.fat30d)+'</td><td data-label="Ult.Entrada">'+x.ult_entrada+'</td></tr>').join('')+'</tbody></table></div>';
